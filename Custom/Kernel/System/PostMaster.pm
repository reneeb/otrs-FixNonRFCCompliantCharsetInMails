...
# Changes (C) 2012-2014 Perl-Services.de, http://perl-services.de
# --
...
# ---
    if ( $Param{Email} ) {
        if ( ! ref $Param{Email} ) {
            $Param{Email} =~ s/charset\s*=\s*/charset=/ig;
        }
        elsif ( ref $Param{Email} eq 'SCALAR' ) {
            ${$Param{Email}} =~ s/charset\s*=\s*/charset=/ig;
        }
        elsif ( ref $Param{Email} eq 'ARRAY' ) {
            $_ =~ s/charset\s*=\s*/charset=/ig for @{$Param{Email}};
        }
    }
# ---
